  ...
  group('气泡物理系统修复验证', () {
    ...
    testWidgets('物理更新后速度应该受到限制', (WidgetTester tester) async {
      ...
      for (final entity in testEntities) {
        ...
        expect(speed, lessThanOrEqualTo(25.0), 
               reason: '实体速度不应超过25.0，当前值: $speed');
    ...
    testWidgets('系统应该能达到近似静止状态', (WidgetTester tester) async {
      ...
      expect(totalEnergy, lessThan(1000.0), 
             reason: '经过长时间模拟，系统能量应该很低。当前能量: $totalEnergy');
    });
    ...
    testWidgets('随机扰动应该是温和的', (WidgetTester tester) async {
      ...
      expect(energyIncrease, lessThan(1000.0), 
             reason: '随机扰动应该是温和的，不应造成剧烈运动。能量增加: $energyIncrease');
    });
  });
}