    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Slider(
            value: (_options.maxDeliveryTime ?? 60).toDouble(),
            min: 15,
            max: 120,
            divisions: 21,
            label: '${_options.maxDeliveryTime ?? 60}分钟内',
            onChanged: (value) {
              _updateFilter(_options.copyWith(maxDeliveryTime: value.round()));
            },
          ),
        ],
      ),
          ...
          Wrap(
            ...
            children: cuisineTypes.map((cuisine) {
              ...
              return FilterChip(
                ...
                selected: isSelected,
                onSelected: (selected) {
                  final currentTypes = List<String>.from(_options.cuisineTypes ?? []);
                  if (selected) {
                    currentTypes.add(cuisine);
                  } else {
                    currentTypes.remove(cuisine);
                  }
                  _updateFilter(_options.copyWith(cuisineTypes: currentTypes));
                },
                selectedColor: Colors.orange,
                backgroundColor: Colors.grey[100],
                checkmarkColor: Colors.white,
