...
/// 智能餐次推荐器
class SmartMealRecommender extends StatelessWidget {
  final Function(MealType) onMealTypeSelected;
  final bool showDescription;

  const SmartMealRecommender({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final recommendedMeals = _getRecommendedMeals();
        ...
        children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (showDescription) ...[
                          const SizedBox(height: 4),
                          Text(
                            _getMealDescription(currentMealType),
                            style: TextStyle(
          ...
          if (recommendedMeals.isNotEmpty) ...[
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: recommendedMeals.map((mealType) {
  ...
  List<MealType> _getRecommendedMeals() {
    final currentMealType = _getCurrentMealType();
    final allMeals = MealType.values.toList();
    allMeals.remove(currentMealType);
    return allMeals.take(2).toList();
  }
