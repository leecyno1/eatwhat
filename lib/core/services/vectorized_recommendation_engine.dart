...
class VectorizedRecommendationEngine {
  ...
  Vector foodToVector(Food food) {
    ...
    // 2. 菜系特征权重
    final cuisineIndex = _featureVocabulary.indexOf(food.cuisineType);
    if (cuisineIndex != -1) {
    ...
    if (food.nutritionFacts != null) {
      _addNutritionFeatures(food.nutritionFacts!, features);
    }
  ...
  Future<List<Food>> getVectorizedRecommendations(
    ...
    print('📊 Top 3 推荐分数: ${scoredFoods.take(3).map((f) => '${f.food.name}:${f.score.toStringAsFixed(3)}').join(', ')}');
  ...
  double _calculateDiversityBonus(Food food, List<VectorizedScoredFood> existingFoods) {
    ...
    // 检查口味多样性
    final existingTastes = existingFoods
        .expand((f) => f.food.tasteAttributes)
        .toSet();
    final newTastes = food.tasteAttributes.where((taste) => !existingTastes.contains(taste));
