...
class FoodDataGenerator {
  ...
  static const List<String> _proteins = [
    '鸡肉', '牛肉', '猪肉', '羊肉', '鱼肉', '虾', '蟹', '鸭肉', '鹅肉', '兔肉',
    '豆腐', '鸡蛋', '鹌鹑蛋', '扇贝', '蛤蜊', '鲍鱼', '海参', '墨鱼', '章鱼', '带鱼'
  ];
  
  static const List<String> _vegetables = [
    '白菜', '菠菜', '韭菜', '芹菜', '萝卜', '胡萝卜', '土豆', '番茄', '黄瓜', '茄子',
    '青椒', '红椒', '洋葱', '大蒜', '生姜', '大葱', '豆角', '冬瓜', '南瓜', '丝瓜',
    '苦瓜', '西兰花', '花菜', '卷心菜', '莴笋', '竹笋', '蘑菇', '木耳', '银耳', '海带'
  ];
  
  static const List<String> _seasonings = [
    '生抽', '老抽', '盐', '糖', '醋', '料酒', '胡椒粉', '花椒', '辣椒', '蒜蓉',
    '豆瓣酱', '甜面酱', '蚝油', '香油', '辣椒油', '花椒油', '芝麻', '花生', '葱花', '香菜'
  ];
  
  // 烹饪方法
  static const List<String> _cookingMethods = [
    '红烧', '清蒸', '爆炒', '干煸', '水煮', '油焖', '糖醋', '宫保', '鱼香', '麻婆',
    '白切', '卤制', '炖煮', '煎制', '烤制', '凉拌', '腌制', '炸制', '熏制', '蒸制'
  ];
  
  // 口味属性
  static const List<List<String>> _tasteProfiles = [
    ['麻', '辣', '鲜'], ['酸', '甜', '鲜'], ['香', '咸', '鲜'], ['清淡', '鲜', '嫩'],
    ['浓郁', '香', '醇'], ['爽脆', '清香'], ['软糯', '甜', '香'], ['鲜美', '滑嫩'],
    ['麻辣', '香'], ['酸辣', '开胃'], ['甜酸', '爽口'], ['咸鲜', '下饭'],
    ['清香', '淡雅'], ['浓香', '厚重'], ['鲜嫩', '多汁'], ['酥脆', '香甜']
  ];
  ...
  static Food _generateSingleFood(String cuisine, int index, List<String> characteristics) {
    ...
    return Food(
      ...
      preparationTime: '${10 + _random.nextInt(50)}分钟', // 10-60分钟
  ...
  static List<Food> generateByPriceRange(double minPrice, double maxPrice, int count) {
    final allFoods = generateFullDatabase();
    final filteredFoods = allFoods.where((food) => 
      food.price != null && food.price! >= minPrice && food.price! <= maxPrice
    ).toList();
